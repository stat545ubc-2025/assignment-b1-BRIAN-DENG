...
2. Example of common errors with this code
```{r, error = TRUE}
...
3. Examples of the code with another dataset
```{r}
...
plot_magenta_histogram(gapminder, lifeExp, 100)
```
4. Example of variable that contains a lot of NA's
```{r}
...
2. Test that the function will not error if there are NA's present 
```{r}
...
3. Test that the function errors if there is nothing in the data frame
```{r}
...
4. Test that a proper ggplot object is outputted
```{r}
